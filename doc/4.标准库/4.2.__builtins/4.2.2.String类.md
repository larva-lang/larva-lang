...
* 除字符串格式化的语法外，其他情形下字符串和一个普通的Larva类没有什么区别，通过句柄引用操作并有自己的方法
...
{
    /*
    构造函数，对String执行new ()操作将构建一个值等同于输入的char数组的字符串，意即长度相等并且对应位置的char值也相等
    */
    public String(char[] arr)

    /*
    实现Stringable接口，返回this本身
    */
    public String str()
    ...
    */
    ...
    /*
    判断字符串this和other的大小关系，采用按char值的字典序计算
    ...
    */
    ...
    - 分割流程：
        ...
        5 截取非空白字符组成的串直到下一个空白字符或者字符串结束，将截取的串加入结果列表，从下一个空白字符的位置继续处理，跳到第2步
    - 例如：
    ...
    */
    ...
    /*
    不限分割部分数量的split_n，相当于this.split_n(sep, LONG_MAX)
    */
    ...
    */
    ...
    合法的double数值字符串是十进制或十六进制的形式，可带正负号前缀，其格式可参考‘3.2.词法元素’中的double类型的浮点数字面量部分
    */
