...
* 对象的比较
    ...
    ```
    /*
    实现了`Equalable<T>`接口的对象可以和`T`的对象通过`eq`方法做判等操作，二者相等返回true，否则返回false
    一般来说，某个类型`Type`若有方法`eq`，都是和它自身判等，即实现`Equalable<Type>`接口
    */
    ...
    /*
    实现了`Comparable<T>`接口的对象可以和`T`的对象通过`cmp`方法做比大小操作，规范是：
    - 若this比other小，则返回负数
    - 若this和other相等，则返回0
    - 若this比other大，则返回正数
    */
...
public class Pair<F, S>
{
    ...
    */
    ...
    /*
    判断this和other是否相等，相等的规则是二者的first和second在泛型函数eq<T>的规范下都相等
    */
...
```
/*
主动使程序崩溃，一般用于不可能出现的或可能造成严重影响的代码bug，必须立即停止程序而非当前fiber
