...
* `RangeError`
    ...
    */
    ...
    需要注意这里的`begin`和`end`的取值可以等于`len`，这是跟下标越界规则不用的地方
...
* `GoError`
    ...
    {
        /*
        返回错误信息格式为：“[GoError]Go的error的Error方法返回的错误”
        */
        public String str()
    ...
    注意这个异常类没有构造方法，因为创建并抛出它的往往是较为底层的Native代码，需要直接采用对`err`字段赋值来创建

    实际开发中一般并不直接使用`GoError`类，因为它的类型信息太泛了，常规做法是对其做封装，例如：
    ```
...
* `GoErrorPorter`
    ...
    `GoErrorPorter`的标识主要作用是可以在Native代码中简化Go的错误和Larva的异常的转换，或一些相关场景下的判断
